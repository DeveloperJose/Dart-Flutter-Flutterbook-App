...
class ContactsList extends StatelessWidget with ImageMixin {
  ...
  Widget build(BuildContext context) {
    return ScopedModel<ContactsModel>(
        model: contactsModel,
        child: ScopedModelDescendant<ContactsModel>(builder: (BuildContext context, Widget child, ContactsModel model) {
          return Scaffold(
                        ...
                        child: ListTile(
                            ...
                            onTap: () async {
                              ...
                              if (contactsModel.entityBeingEdited.birthday == null) {
                                ...
                                contactsModel.setBirthday(utils.toFormattedDate(contactsModel.entityBeingEdited.birthday));
                              }
                              contactsModel.setStackIndex(1);
                            }),
  ...
  Future _deleteContact(BuildContext context, Contact contact) {
    return showDialog(
        ...
        builder: (BuildContext alertContext) {
          return AlertDialog(title: Text('Delete Contact'), content: Text('Really delete ${contact.name}?'), actions: [
            FlatButton(
              child: Text('Cancel'),
              onPressed: () {
